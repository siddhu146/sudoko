...
document.querySelector('.input').innerHTML='Select the size of Matrix';
...
function satisfy(matrix,x,y,num,n){
    //checking whether the number that system has entered is valid
    //checking the particular row and column
    for(let k=0;k<n;k++){
        if(matrix[x][k]==num||matrix[k][y]==num){
            return false;
        }
    }
